...
for i = 1:numel(folder_names)
    for j = 1:numel(frame_folders)  
        ...
        for k = 1:video_counts(i,j)
            filename1 = [folder_name '_' frame_folder '_vid' num2str(k) '_p_ref.jpg'];
            filename2 = [folder_name '/' frame_folder '/vid' num2str(k) '/' folder_name '_' frame_folder '_vid' num2str(k) '_p_ref.jpg'];
            try
                I = imread(filename2);
                ...
                % 检查 ${folder_name}/${frame_folder}/vid${k}/p 目录下有多少个帧
                p_frame_num = numel(dir([folder_name '/' frame_folder '/vid' num2str(k) '/p/*.jpg']));
                disp([folder_name '/' frame_folder '/vid' num2str(k) '/p目录里有' num2str(p_frame_num) '帧']);
                extract([folder_name '/' frame_folder '/vid' num2str(k) '/p'],p_frame_num);
                copyfile('tmp/i.jpg',[folder_name '/' frame_folder '/vid' num2str(k) '/' folder_name '_' frame_folder '_vid' num2str(k) '_p_ref.jpg']); 
                rmdir('tmp','s');
    
    
                % End timer and display elapsed time  
                time = toc;
                disp(['Run ' folder_name '/' frame_folder '/vid' num2str(k) '/p' ': Elapsed time is '  num2str(time) ' seconds.']);  
    
                % Open CSV file and write data
                file = fopen(filename,'a');  
                fprintf(file,'%s/%s/vid%s/p, %fs\n',folder_name,frame_folder,num2str(k),time);  
                fclose(file);
            end
        end
    end
